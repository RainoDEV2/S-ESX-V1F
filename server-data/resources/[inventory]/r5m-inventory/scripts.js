...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://r5m-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            itemList = item.list;
        } else if (item.response == 'sendData') {
            charName = item.list;
        } else if (item.response == 'GiveItemChecks') {
            if (itemList[item.id]) {
                $.post(
                    'https://r5m-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function ToggleBar(toggle, boundItems, boundItemsAmmo) {
    if (toggle) {
        ...
        if (boundItems[1]) {
            ...
            htmlstring =
                "<div id='bind1'> 1 </div> <div class='item3' > <div class='itemname3'> boş </div> <img src='icons/empty.png' class='itemimage'>  </div>";
        }
        ...
        for (let i = 2; i < 6; i++) {
            if (boundItems[i]) {
                ...
                htmlstring =
                    ...
                    "'> " +
                    i +
                    " </div><div class='item3' > <div class='itemname3'> boş </div> <img src='icons/empty.png' class='itemimage'>  </div>";
            }
        }
...
function invStack(
    ...
    $.post('https://r5m-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://r5m-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://r5m-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://r5m-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
let MyInventory = {};
let MyItemCount = 0;
let exeTargetInventory = {};
let exeTargetCount = 0; 
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    exeTargetInventory = targetinventory;
    exeTargetCount = targetitemCount;
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        slotLimitTarget = 30;
        displayName = 'Yer';
    } else if (targetinvName.indexOf('Motel') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 30;
        displayName = 'Motel Odası';
    } else if (targetinvName.indexOf('StashEV') > -1) {
        secondaryMaxWeight = 2000.0;
        slotLimitTarget = 30;
        displayName = 'Stash House';
    } else if (targetinvName.indexOf('Kiyafet') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 15;
        displayName = 'Kıyafetlerim';
    } else if (targetinvName.indexOf('Kanit') > -1) {
        secondaryMaxWeight = 500.0;
        slotLimitTarget = 20;       
        displayName = 'Kanıt Envanteri '+(targetinvName).split("_")[1];
    } else if (targetinvName.indexOf('Çöp') > -1) {
        secondaryMaxWeight = 1000.0;
        slotLimitTarget = 30;
        displayName = 'Çöplük';
    } else if (targetinvName.indexOf('Burger') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 15;
        displayName = 'Burger Masa';
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://r5m-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
    ...
    for (i = 0; i < slotLimit + 1; i++) {
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                $.post("http://r5m-inventory/updateMyQuality", JSON.stringify({
                    item_id: itemid,
                    slot: slot,
                    quality: quality
                }));
...
function UpdateSetWeights(secondaryName) {
    ...
    if (!$('#playerWeight').html() || $('#secondaryInvName').attr('title') !== TargetInventoryName) {
        $('#wrapPersonalWeight').html(
            "<h2 title=''> [" + charName.id + "] " + charName.name + " - " + charName.job + "</h2>" +
...
$(document).on('mousedown mouseup', (event) => {
    ...
    if (element.id === 'CloseInv') {
        ...
        useitem();
        return;
    } else if (element.id === 'give') {
        if (!isDragging) return;
        giveItem();
        return;
    } else if (element.id === 'kiyafetlerim') {
        if (isDragging) {
            EndDrag(draggingid);
        }
        closeInv();      
        $.post('https://r5m-inventory/openClothesInv');
        $(usearea).hide();
        $('.inventory').css('background-color', 'rgba(1, 1, 1, 0.0)');
        $('.wrapsecondary').css('float', 'left');
        $('.wrapsecondary').css('width', '28%');
        $('.wrapmain').css('width', '28%');
        $('.wrapmain').css('margin-left', '5%');
        $('.wrapPersonalWeight').css('left', '5%');
        $('.wrapPersonalWeight').css('width', '25%');
        $('.wrapSecondaryWeight').hide();
        return;
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://r5m-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://r5m-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://r5m-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://r5m-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://r5m-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {   
    // $.post('https://r5m-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://r5m-inventory/ServerCloseInventory',
        JSON.stringify({
    ...
    $.post(
        'https://r5m-inventory/Close',
        JSON.stringify({
            isItemUsed: pIsItemUsed,
        }),
    );
    $(usearea).show();
    $('.inventory').css('background-color', 'rgba(1, 1, 1, 0.5)');
    $('.wrapsecondary').css('float', 'right');
    $('.wrapsecondary').css('width', '31%');
    $('.wrapmain').css('width', '31%');
    $('.wrapmain').css('margin-left', '10%');
    $('.wrapPersonalWeight').css('left', '10%');
    $('.wrapPersonalWeight').css('width', '31%');
    $('.wrapSecondaryWeight').show();
}
...
function AttemptDropInFilledSlot(slot) {
    ...
    if (
        ...
        result = 'Mağazaya eşya bırakamazsınız!';
    }
    if (
        ...
        result = 'Eşyaları üretim masasına bırakamazsınız!';
    }
    ...
    if (result == 'Success' && result2 == 'Success') {
        ...
        if (stacking) {
            ...
            if (inventoryDropName == 'Shop' || inventoryDropName == 'Craft' || (!StoreOwner && PlayerStore)) {
                ...
                if (TargetInventoryName.indexOf('Kiyafet') > -1) {
                    InventoryLog("Kıyafetinizi değiştirmek için boş slota sürükleyiniz");
                    EndDragError(slot);
                    return;
                }
                SwapStacks(
...
function AttemptDropInEmptySlot(slot, isDropped, half) {
    ...
    let targetInventory = JSON.parse(exeTargetInventory);

    if (TargetInventoryName.indexOf('Kiyafet') > -1 && inventoryDropName === 'wrapsecondary' && inventoryReturnItemDropName === 'wrapmain') {   
        var verilebilecekler = {1:"bags", 2:"tshirt", 3:"torso", 4:"chain", 5:"pants", 6:"shoes", 7:"arms", 8:"mask", 9:"glasses", 10:"bracelets", 11:"watches", 12:"helmet", 13:"ears", 14:"bproof", 15:"decals"}
        var found = false

        for (var i = 1; i <= 15; i++) {
            if (itemidsent === verilebilecekler[i])
            found = true
        }  

        if (found === true){
            if (exeTargetCount === 0) {
                targetInventory[exeTargetCount] = {item_id : itemidsent};
                exeTargetInventory = JSON.stringify(targetInventory);
                exeTargetCount = exeTargetCount + 1;
                $.post('https://r5m-inventory/wearClothes', JSON.stringify({
                    item: itemidsent,
                    info: item.dataset.info,
                }));
            } else {
                var tempCount = exeTargetCount
                for (let i = 0; i < parseInt(tempCount); i++) {
                    if (targetInventory[i].item_id === itemidsent) {
                        InventoryLog('Zaten bu kıyafeti giyiyorsunuz!'); 
                        EndDragError(slot);
                        return; 
                    }
                } 
            ...
            InventoryLog('Buraya sadece kıyafet koyabilirsin');
            EndDragError(slot);
            return;
        }          
    }

    if (TargetInventoryName.indexOf('Kiyafet') > -1 && inventoryDropName === 'wrapmain' && inventoryReturnItemDropName == "wrapsecondary") {
        targetInventory = JSON.parse(exeTargetInventory);
        var tempCount = exeTargetCount
        for (let i = 0; i < parseInt(tempCount); i++) {
            if (targetInventory[i].item_id === itemidsent) {
                targetInventory[i].item_id = "bos";
                exeTargetInventory = JSON.stringify(targetInventory);
                exeTargetCount = exeTargetCount - 1 
            }
        }
        $.post('https://r5m-inventory/takeOffClothes', JSON.stringify({
            item: itemidsent,
            info: item.dataset.info,
        }));
    }
    ...
    if (isDropped && inventoryReturnItemDropName == 'wrapsecondary') {
        result = 'Error: düşürülmüş bir eşya düşürülemez';
    }
    ...
    if (result == 'Success') {
        ...
        if (currentInventory == 1 && (inventoryDropName == 'wrapsecondary' || isDropped)) {
            ...
            $.post('https://r5m-inventory/checkWeapon', JSON.stringify(itemidsent));
        }
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName && currentInventory != 2) {
        let arr = [inventoryUsedName, itemid, slotusing, isWeapon, itemusinginfo];
        $.post('https://r5m-inventory/invuse', JSON.stringify(arr));
...
function giveItem() {
    ...
    if (inventoryUsedName == PlayerInventoryName && currentInventory != 2) {
        let amountValue = $('#move-amount').val();
        let arr = [itemList[itemid].displayname, amountValue, itemid, slotusing];
        $.post('https://r5m-inventory/giveitem', JSON.stringify(arr));
        //InventoryLog("Using item: " + name + "(" + amount + ") from " + inventoryUsedName + " | slot " + slotusing)
    }
    EndDrag(slotusing);
    closeInv(true);
