...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if tonumber(ItemInfo.quality) ~= nil then
     if tonumber(ItemInfo.quality) < 1 then
         TriggerEvent("notification","Bu eşya kullanılamaz halde", 2) 
         if isWeapon then
       ...
       -- local animasyon = armourAnim()
        if armorValue < maxArmorValue then
           ...
           -- local finished = exports["np-taskbar"]:taskBar(10000,"Armor",true,false)
            --if (finished == 100) then
            armourAnim()
            exports["np-taskbar"]:taskBar(10000,"Zırh Giyiyorsun")--TaskItem(animasyon, "Zırh Kullanıyorsun","r5m-hospital:items:bandage",true,itemid,playerVeh)
    ...
    if (itemid == "oxygentank") then
        ...
        if (finished == 100) then        
            ...
            TriggerEvent('esx_tenues:settenueplongee', _source)
            TriggerEvent('sualti:basla', _source)
            remove = true
        end
    end
    ...
    if (itemid == "bandage") then
        ...
        if health < maxHealth then
            TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Bandaj Kullanıyorsun","r5m-hospital:items:bandage",true,itemid,playerVeh)
    ...
    if (itemid == "IFAK") then
        ...
        local finished = exports["np-taskbar"]:taskBar(5000,"Applying IFAK",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "cigarette") then
        if getQuantity("lighter") > 0 then
            ...
            TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Çakmağın yok!', length = 4500, style = { ['background-color'] = '#ff0000', ['color'] = '#ffffff' } })
        end
    end
    ...
    for i,v in pairs(clientInventory) do
        ...
        if amount > 0 then
            ...
            for i=1, #userData.items, 1 do
                ...
                if invitem.name == v.item_id then
                    founded = true
                end
            end
            if not founded then
...
RegisterCommand("soy", function(source, args)
	...
	if closestPlayer ~= -1 and closestDistance <= 1.0 then
		...
		if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(searchPlayerPed, 'dead', 'dead_a', 3) then
			...
			TriggerEvent('notification', 'Kişi ellerini kaldırmamış')
		end
	end
end)
...
RegisterCommand("paracal", function(source, args)
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        if PlayerData.job.name == 'police' then
            local searchPlayerPed = GetPlayerPed(closestPlayer)
            ...
            TriggerEvent("notification", "Polis Değilsin Dallama")
        end
    end
end)
    ...
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        ...
        if cinsiyet == info.Cinsiyet then
            ...
            --[[if item == "bags" then
                ...
                Citizen.Wait(3000)
                rygPalette = GetPedPaletteVariation(playerPed, 5) 
                SetPedComponentVariation(playerPed, 5, info.Numara, info.Renk, rygPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "tshirt" then
                ...
                Citizen.Wait(1300)
                undershirtPalette = GetPedPaletteVariation(playerPed, 8) 
                SetPedComponentVariation(playerPed, 8, info.Numara, info.Renk, undershirtPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "torso" then
                ...
                TaskPlayAnim(playerPed, "clothingshirt", "check_out_b", 3.5, -8, -1, 49, 0, 0, 0, 0)
                Citizen.Wait(1300)
                shirtPalette = GetPedPaletteVariation(playerPed, 11) 
                SetPedComponentVariation(playerPed, 11, info.Numara, info.Renk, shirtPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "chain" then
                ...
                Citizen.Wait(5000)
                halsPalette = GetPedPaletteVariation(playerPed, 7) 
                SetPedComponentVariation(playerPed, 7, info.Numara, info.Renk, halsPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "pants" then
                ...
                Citizen.Wait(3000)
                bukserPalette = GetPedPaletteVariation(playerPed, 4) 
                SetPedComponentVariation(playerPed, 4, info.Numara, info.Renk, bukserPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "shoes" then
                ...
                Citizen.Wait(1300)
                skoPalette = GetPedPaletteVariation(playerPed, 6) 
                SetPedComponentVariation(playerPed, 6, info.Numara, info.Renk, skoPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "arms" then
                ...
                Citizen.Wait(1300)
                GlovesPalette = GetPedPaletteVariation(playerPed, 3) 
                SetPedComponentVariation(playerPed, 3, info.Numara, info.Renk, GlovesPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "mask" then
                ...
                Citizen.Wait(1000)
                MaskPalette = GetPedPaletteVariation(playerPed, 1) 
                SetPedComponentVariation(playerPed, 1, info.Numara, info.Renk, MaskPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "glasses" then
                ...
                SetPedPropIndex(playerPed, 1, info.Numara, info.Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "bracelets" then
                ...
                SetPedPropIndex(playerPed, 7, info.Numara, info.Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "watches" then
                ...
                SetPedPropIndex(playerPed, 6, info.Numara, info.Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "helmet" then
                ...
                SetPedPropIndex(playerPed, 0, info.Numara,info.Renk, true)
                Citizen.Wait(400)
                ClearPedTasks(playerPed)
            ...
            elseif item == "ears" then
                ...
                SetPedPropIndex(playerPed, 2, info.Numara, info.Renk, true)
                ClearPedTasks(playerPed)
            ...
            if skin.sex == 0 then
                TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Bu kıyafet kadınlar için tasarlanmış', length = 6000, style = { ['background-color'] = '#CC0000', ['color'] = '#FFFFFF' } })
            else
                TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Bu kıyafet erkekler için tasarlanmış', length = 6000, style = { ['background-color'] = '#CC0000', ['color'] = '#FFFFFF' } })
            end
        end
    ...
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        ...
        if cinsiyet == info.Cinsiyet then
            ...
            if item == "bags" then
                ...
                Citizen.Wait(3000)
                rygPalette = GetPedPaletteVariation(playerPed, 5)
                SetPedComponentVariation(playerPed, 5, numbers["bags"].Numara, numbers["bags"].Renk, rygPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "tshirt" then
                ...
                Citizen.Wait(1300)
                GlovesPalette = GetPedPaletteVariation(playerPed, 3)
                SetPedComponentVariation(playerPed, 3, numbers["arms"].Numara, numbers["arms"].Renk, GlovesPalette)
                TriggerServerEvent("ExeLds:saveSkin","arms", info, numbers)
                undershirtPalette = GetPedPaletteVariation(playerPed, 8)
                SetPedComponentVariation(playerPed, 8, numbers["tshirt"].Numara, numbers["tshirt"].Renk, undershirtPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "torso" then
                ...
                Citizen.Wait(1300)
                shirtPalette = GetPedPaletteVariation(playerPed, 11)
                SetPedComponentVariation(playerPed, 11, numbers["torso"].Numara, numbers["torso"].Renk, shirtPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "chain" then
                ...
                Citizen.Wait(5000)
                halsPalette = GetPedPaletteVariation(playerPed, 7)
                SetPedComponentVariation(playerPed, 7, numbers["chain"].Numara, numbers["chain"].Renk, halsPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "pants" then
                ...
                Citizen.Wait(3000)
                bukserPalette = GetPedPaletteVariation(playerPed, 4)
                SetPedComponentVariation(playerPed, 4, numbers["pants"].Numara, numbers["pants"].Renk, bukserPalette)
                ClearPedTasks(playerPed)
                TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Kolye çıkarıldı', length = 6000, style = { ['background-color'] = '#CC0000', ['color'] = '#FFFFFF' } })
            ...
            elseif item == "shoes" then
                ...
                Citizen.Wait(1300)
                skoPalette = GetPedPaletteVariation(playerPed, 6)
                SetPedComponentVariation(playerPed, 6, numbers["shoes"].Numara, numbers["shoes"].Renk, skoPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "mask" then
                ...
                Citizen.Wait(500)
                MaskPalette = GetPedPaletteVariation(playerPed, 1)
                SetPedComponentVariation(playerPed, 1, numbers["mask"].Numara, numbers["mask"].Renk, MaskPalette)
                ClearPedTasks(playerPed)
            ...
            elseif item == "glasses" then
                ...
                Citizen.Wait(4000)
                ClearPedProp(playerPed, 1)
                SetPedPropIndex(playerPed, 1, numbers["glasses"].Numara, numbers["glasses"].Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "bracelets" then
                ...
                Citizen.Wait(1300)
                ClearPedProp(playerPed, 7)
                SetPedPropIndex(playerPed, 7, numbers["bracelets"].Numara, numbers["bracelets"].Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "watches" then
                ...
                Citizen.Wait(1300)
                ClearPedProp(playerPed, 6)
                SetPedPropIndex(playerPed, 6, numbers["watches"].Numara, numbers["watches"].Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "helmet" then
                ...
                Citizen.Wait(1300)
                ClearPedProp(playerPed, 0)
                SetPedPropIndex(playerPed, 0, numbers["helmet"].Numara,numbers["helmet"].Renk, true)
                ClearPedTasks(playerPed)
            ...
            elseif item == "ears" then
                ...
                Citizen.Wait(1300)
                ClearPedProp(playerPed, 2)
                SetPedPropIndex(playerPed, 2, numbers["ears"].Numara, numbers["ears"].Renk, true)
                ClearPedTasks(playerPed)
...
AddEventHandler('ExeLds:openClothesInv', function()
    TriggerServerEvent("ExeLds:openRoomStorage", "Kiyafet")
end)
...
RegisterNetEvent('ExeLds:getCount')
AddEventHandler('ExeLds:getCount', function(item)
    local count = getQuantity(item)
    TriggerServerEvent("ExeLds:itemCount", item, count)
end)
