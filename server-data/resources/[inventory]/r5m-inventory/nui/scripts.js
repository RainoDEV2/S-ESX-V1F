...
function ToggleBar(toggle, boundItems, boundItemsAmmo) {
    if (toggle) {
        ...
        if (boundItems[1]) {
            ...
            htmlstring =
                "<div id='bind1'> 1 </div> <div class='item3' > <div class='itemname3'> empty </div> <img src='icons/empty.png' class='itemimage'>  </div>";
        }
        ...
        for (let i = 2; i < 6; i++) {
            if (boundItems[i]) {
                ...
                htmlstring =
                    ...
                    "'> " +
                    i +
                    " </div><div class='item3' > <div class='itemname3'> empty </div> <img src='icons/empty.png' class='itemimage'>  </div>";
            }
        }
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        slotLimitTarget = 15;
        displayName = 'my clothes';
    } else if (targetinvName.indexOf('Kanit') > -1) {
        secondaryMaxWeight = 500.0;
        slotLimitTarget = 20;       
        displayName = 'Evidence Inventory '+(targetinvName).split("_")[1];
...
function AttemptDropInFilledSlot(slot) {
    ...
    if (
        ...
        result = 'You cannot leave items in the store!';
    }
    if (
        ...
        result = 'You cannot leave items on the crafting table!';
    }
        ...
        if (stacking) {
            ...
            if (inventoryDropName == 'Shop' || inventoryDropName == 'Craft' || (!StoreOwner && PlayerStore)) {
                ...
                if (TargetInventoryName.indexOf('Kiyafet') > -1) {
                    InventoryLog("Drag it to the empty slot to change your outfit");
                    EndDragError(slot);
                    return;
                }
                SwapStacks(
...
function AttemptDropInEmptySlot(slot, isDropped, half) {
    ...
    if (TargetInventoryName.indexOf('Kiyafet') > -1 && inventoryDropName === 'wrapsecondary' && inventoryReturnItemDropName === 'wrapmain') {   
        ...
        if (found === true){
            if (exeTargetCount === 0) {
                ...
                for (let i = 0; i < parseInt(tempCount); i++) {
                    if (targetInventory[i].item_id === itemidsent) {
                        InventoryLog('You are already wearing this outfit!'); 
                        EndDragError(slot);
                        return; 
                    }
                } 
            ...
            InventoryLog('You can only put clothes here');
            EndDragError(slot);
            return;
        }          
    }
    ...
    if (isDropped && inventoryReturnItemDropName == 'wrapsecondary') {
        result = 'Error: You have already done this action';
    }
    ...
    if (
        ...
        result = 'You cannot leave items here!';
    }
    ...
    if (inventoryDropName == 'wrapsecondary' && PlayerStore) {
        ...
        if (isWeapon != undefined) {
            result = 'You cannot move weapons while in player stores!';
        }
    }
...
function giveItem() {
    ...
    if (inventoryUsedName == PlayerInventoryName && currentInventory != 2) {
        let amountValue = $('#move-amount').val();
        let arr = [itemList[itemid].displayname, amountValue, itemid];
