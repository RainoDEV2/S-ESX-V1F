...
on("__cfx_nui:Weight", (data, cb) => {
	...
	emit("updateCurrentWeight", personalWeight);
})
RegisterNuiCallbackType("Close");
on("__cfx_nui:Close", (data, cb) => {
    CloseGui(data.isItemUsed)
	emit("ExeLds:removePed");
})
...
on("__cfx_nui:GiveItem", (data, cb) => {
	...
	GiveItem(id, amount, generateInformation,nonStacking, itemdata, isWeapon)	
})
...
on('player:receiveItem', (id, amount,generateInformation, itemdata) => {
	if (personalWeight > 250) {
		emit("notification",id + " You exceeded the weight limit")
		return
	}
...
function findSlot(ItemIdToCheck,amount, nonStacking) {
	...
	if (foundslot == 0) {
		emit("notification","Failed to give " + ItemIdToCheck + " full of inventory",2)
	}
...
RegisterNetEvent('Getfirstitemscunt')
on('Getfirstitemscunt', () => {
	cid = exports.isPed.isPed("cids")
	if (openedInv) {
	}else{
...
on('inventory-open-request', () => {
	...
	/*} else if (BinFound) {
		...
		if (lockStatus == 1 || lockStatus == 2) {
			if (lockStatus == 2 && distanceRear < 1.5) {
				emit("ExeLds:sendNotify", "vehicle locked");
...
RegisterNetEvent('inventory-set-ready-request')
on('inventory-set-ready-request', () => {
	SendNuiMessage(JSON.stringify({ response: "SendItemList", list: itemList}))
	cid = exports.isPed.isPed("cids")
...
function setInventoryReady() {
	emitNet("server-inventory-setReady", cid);
}
...
function drawMarkersUI() {
	for (let Row in DrawInventories) {		
		DrawMarker(20,DrawInventories[Row].position.x, DrawInventories[Row].position.y, DrawInventories[Row].position.z-0.8,0,0,0,0,0,0,0.35,0.5,0.15,43, 131, 225,200,0,0,0,0)
	}	
}
...
on('inventory-bind', (slot) => {
	...
	let cid = exports.isPed.isPed("cids")
...
RegisterNetEvent('SetReadyInv')
on('SetReadyInv', () => {
	emit('inventory-set-ready-request')
});

RegisterNetEvent('ExeLds:sendData')
on('ExeLds:sendData', (name) => {
	SendNuiMessage(JSON.stringify({ response: "sendData", list: name}))
});
...
on("__cfx_nui:updateMyQuality", (data, cb) => {
	let cid = exports.isPed.isPed("cids")
	emitNet("server-item-quality-update", 'ply-' + cid, data)
})
...
function GiveItem(itemid,amount, generateInformation, nonStacking, itemdata, isWeapon) {
	...
	if (slot != 0) {
		cid = exports.isPed.isPed("cids")
		emitNet("server-inventory-give",cid,itemid,slot,amount, generateInformation, Object.assign({}, itemdata), openedInv, isWeapon)
...
RegisterNuiCallbackType('giveitem')
on('__cfx_nui:giveitem', (data) => {
	let item = data[0] 
	let amount = data[1]
	let itemid = data[2]	
	let slot = data[3]

	emit("GiveItem",item, amount, itemid, slot);
});


RegisterNuiCallbackType('checkWeapon')
on('__cfx_nui:checkWeapon', (data) => {
	emit("ExeLds:weaponCheck", data)
});

RegisterNuiCallbackType('wearClothes')
on('__cfx_nui:wearClothes', (data) => {
	emit("ExeLds:wearClothes", data.item, data.info)
});

RegisterNuiCallbackType('openClothesInv')
on('__cfx_nui:openClothesInv', () => {
	emit("ExeLds:openClothesInv")
});

RegisterNuiCallbackType('takeOffClothes')
on('__cfx_nui:takeOffClothes', (data) => {
	emit("ExeLds:takeOffClothes", data.item, data.info)
});
