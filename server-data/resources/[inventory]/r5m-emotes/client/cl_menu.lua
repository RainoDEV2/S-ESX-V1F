...
local Keys = {
    [170] = "F3", [166] = "F5", [167] = "F6", [168] = "F7" ,[56] = "F9", [57] = "F10" 
}
...
AddEventHandler("emotes:OpenMenu", function()
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        TriggerEvent("notification", "Cant access this when in a car", 2)
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
      if IsDisabledControlJustPressed(1, 170) then	--SHIFT AND P
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
            TriggerEvent("emotes:OpenMenu")
        else
            TriggerEvent("notification", "Cant access this when in a car", 2)
