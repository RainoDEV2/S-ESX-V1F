...
anims = {
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["r5m-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["r5m-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["smoke"] = function(ped)
        if exports["r5m-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["r5m-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                SetCurrentPedWeapon(ped, 0xA2719263, true)
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or animName == "weights") and
        (#(GetEntityCoords(PlayerPedId()) - vector3(-1200.8356933594,-1567.9475097656,4.6119947433472)) < 10.0 or
        #(GetEntityCoords(PlayerPedId()) - vector3(-1220.5383300781,-1544.6274414063,4.6974620819092)) < 10.0) then
        local finished = exports["r5m-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
...
AddEventHandler('notepad', function()
    ...
    if GetPedInVehicleSeat(veh, -1) ~= PlayerPedId() then
        ...
        Citizen.Wait(500)
        while IsEntityPlayingAnim(PlayerPedId(), "amb@medic@standing@timeofdeath@base", "base", 3) do Citizen.Wait(1) end
...
RegisterNetEvent('r5m-fines:Anim')
AddEventHandler('r5m-fines:Anim', function()
