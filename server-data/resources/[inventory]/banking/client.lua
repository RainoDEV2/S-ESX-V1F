...
AddEventHandler('bank:checkATM', function()
  if IsNearATM() then
    ...
    TriggerEvent("notification","No ATMs nearby!", 2)
  end
end)
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["np-taskbar"]:taskBar(3000,"Don't forget to get your card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["np-taskbar"]:taskBar(1000,"Don't forget to get your card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
          ...
          while cdst < 1.5 do
            ...
            cdst = #(vector3(banks[scanid].x, banks[scanid].y, banks[scanid].z) -  vector3(plyCoords["x"], plyCoords["y"], plyCoords["z"]))
              ...
              if ClosedBanks[scanid] then
                DrawText3D(banks[scanid].x, banks[scanid].y, banks[scanid].z,"Banka kapalıdır.")
              else
                 DrawText3D(banks[scanid].x, banks[scanid].y, banks[scanid].z,"~b~"..Controlkey["generalUse"][2].."~w~ - Veznede isleme basla")
                atBank = true
...
AddEventHandler('bank:deposit', function(amount)
    if (IsInVehicle()) then
      TriggerEvent("notification", "Araçtayken ATM'yi kullanamazsınız.", 2)
    else
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true) then
    if (IsInVehicle()) then
      ...
      TriggerEvent("notification", "You cannot use the bank while in the vehicle.", 2)
...
AddEventHandler('dirtyMoney:givedm', function(toPlayer, amount)
  if not IsNearPlayer(toPlayer) then 
    TriggerEvent("notification", "You are not near the player", 2)
    return 
  end
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent("notification", "Bu hesaba ulaşamıyorum.", 2)
  end
end)
...
RegisterCommand("nakitbak", function(src, args, raw)
...
AddEventHandler('bank:givecash', function(sender, target, amount)
  ...
  if closestPlayer == -1 or closestDistance > 3.0 then
    TriggerEvent("notification", "There is no one by your side.", 2)
  else
