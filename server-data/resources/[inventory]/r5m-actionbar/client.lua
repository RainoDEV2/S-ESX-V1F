...
RegisterNetEvent("r5m:playerBecameJob")
AddEventHandler("r5m:playerBecameJob", function(job, name, notify)
    currentJob = job
end)
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["r5m-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["r5m-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID)	
	...
	TriggerServerEvent("ExeLds:updateLastWeaponSerial", currentInformation.serial)
...
RegisterCommand("sametmermi",function()
	local ped = PlayerPedId()
	local hash = GetSelectedPedWeapon(ped)
	local newammo = 0
	local ammoType = Citizen.InvokeNative(0x7FEAD38B326B9F74, ped, hash)
...
AddEventHandler('actionbar:ammo', function(hash,amount,addition,itemid)
	...
	curAmmo = GetAmmoInPedWeapon(ped, weaponHash)
	ammoTable["" .. ammoType .. ""]["ammo"] = curAmmo
	...
	if tonumber(curAmmo) >= 150 then
		TriggerEvent('mythic_notify:client:SendAlert',  { type = 'inform', text = 'Your gun is loaded', length = 6000, style = { ['background-color'] = '#CC0000', ['color'] = '#FFFFFF' } })
	elseif newammo > 150 then
		newammo = 150	
		TriggerEvent("inventory:removeItem",itemid, 1)
		local finished = exports["np-taskbar"]:taskBar(3000,"Reload Weapon",false,true)
    	if (finished == 100) then
			ammoTable["" .. ammoType .. ""]["ammo"] = newammo
			SetPedAmmo(PlayerPedId(), weaponHash, newammo)
			prevupdate = 0
			updateAmmo()
...
RegisterCommand('test333', function(source)
...
AddEventHandler('armory:ammo', function()
	...
	TriggerEvent("actionbar:ammo",1742569970,150,true)
end)
...
RegisterNetEvent('r5m-item:CheckClientAmmo')
AddEventHandler('r5m-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
RegisterCommand('hotbar', function(source, args, raw)
	if tabac then
		tabac = false
		exports['mythic_notify']:SendAlert('inform', 'Hotbar kapatıldı.', 3000, { ['background-color'] = '#CC0000', ['color'] = '#FFFFFF' })
		TriggerEvent("inventory-bar",false)
	else
		tabac = true
		exports['mythic_notify']:SendAlert('inform', 'Hotbar aktif edildi.', 3000, { ['background-color'] = '#00CC00', ['color'] = '#000000' })
		TriggerEvent("inventory-bar",true)
	end
end)
...
function grab2h(weaponHash)
	...
	SetCurrentPedWeapon(ped, weaponHash, 1)
    ClearPedTasks(ped)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'bufeci' then
